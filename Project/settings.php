...
if (isset($_POST['theme'])) {
    $themePreference = $_POST['theme']; // This will be either 'light' or 'dark'
    $userId = $_SESSION['user_id']; // This should be the user's ID, not username.

    $query = "UPDATE user_form SET theme = ? WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("si", $themePreference, $userId);
    if ($stmt->execute()) {
        $_SESSION['theme'] = $themePreference; // Update the session variable to the new theme
...
<div class="main-content">
    <?php if ($success): ?> <!-- main box for change password and theme -->
    ...
    <form method="POST" action=""> <!-- allows user to change password -->
    ...
    <form method="POST" action=""> <!-- allows user to select between light and dark mode-->
