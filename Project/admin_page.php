...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_message'])) {
    ...
    // Assuming the admin_id is stored in the session
    $admin_id = isset($_SESSION['admin_id']) ? $_SESSION['admin_id'] : null;

    // Prepare the SQL statement
    $stmt = $conn->prepare("INSERT INTO notifications (message, admin_id) VALUES (?, ?)");
    $stmt->bind_param("si", $admin_message, $admin_id);
...
<div class="main-content">
    ...
    <form action="admin_page.php" method="post">
        <textarea name="admin_message" rows="4" cols="50" placeholder="Enter your message here"></textarea>
...
<script>
    document.querySelectorAll('td[contenteditable="true"]').forEach(cell => {
        cell.addEventListener('input', function () {
            ...
            const isNoShifts = cellContent.toLowerCase() === 'delete';
            ...
            if (timePattern.test(cellContent)) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert('Shift already exists for this date. Please type "delete" in the specified box you are attempting to changing.');
                        window.location.reload();
                    }
                })
                .catch((error) => {
